...
b_conv5 = tf.get_variable('b_conv5', shape=[32])
...
b_fc7 = tf.get_variable('b_fc7', shape=[512])
...
optimizer = tf.train.RMSPropOptimizer(learning_rate=LR).minimize(mean_loss)

augmentation_seq = iaa.Sequential([iaa.Crop(px=(0, 16), name="Cropper"),
                                   iaa.Fliplr(0.5, name="Flipper"),
                                   iaa.GaussianBlur((0, 3.0), name="GaussianBlur"),
                                   iaa.Dropout(0.02, name="Dropout"),
                                   iaa.AdditiveGaussianNoise(scale=0.01 * 255, name="GaussianNoise"),
                                   iaa.Affine(translate_px={"x": (IMG_SIZE // 3, IMG_SIZE // 3)}, name="Affine: TranslateX"),
                                   iaa.Affine(rotate=(-10, 10), name="Affine: Rotate")])
...
with tf.Session() as sess:
    ...
    for epoch_i in range(EPOCHS):
        ...
        for batch_i in range(dataset.num_batches_in_epoch()):
            ...
            X_batch = augmentation_seq_deterministic.augment_images(X_batch)
